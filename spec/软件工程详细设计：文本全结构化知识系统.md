...
| L4 | Community | SCHEMAFULL TABLE | 社区（聚类簇） |
| L4 | Process | SCHEMAFULL TABLE | 流程节点 |
| L4 | ProcessStep | SCHEMAFULL EDGE | 流程步骤边 |

> **设计说明**：L4 层实体（Community/Process/ProcessStep）在代码实现中已扩展，
> 用于支持语义知识图谱的社区发现和流程建模。详见 `knowledge-core/src/model/` 中的
> `Community`、`Process`、`ProcessStep` 结构体及 `semantic.rs` 中的语义模型。
...
-- 注意：代码实现中，embedding 已解耦为独立的 BlockEmbedding 结构体，
-- Block 本身不再包含 embedding 字段。这避免了序列化/反序列化时携带 ~6KB 向量数据。
-- Schema 中保留此字段定义以兼容 SurrealDB 向量索引查询。
-- 详见 knowledge-core/src/model/mod.rs 中 Block/BlockEmbedding 的分离设计。
...
> **设计说明**：以下模型在代码实现中已完整实现，用于支持语义级别的知识图谱构建。
> 详见 `knowledge-core/src/model/semantic.rs` 和 `knowledge-core/src/model/community_summary.rs`。
...
| description | Option\<String\> | 实体描述 |
| aliases | Vec\<String\> | 别名列表 |
| embedding | Option\<Vec\<f64\>\> | 语义向量（概念-载体分离建议：拆分为独立 SemanticEntityEmbedding） |
| confidence | f64 | 置信度 [0.0, 1.0] |
| source_document | Option\<String\> | 来源文档 ID |
| source_block | Option\<String\> | 来源块 ID |
| community_id | Option\<String\> | 所属社区 ID |

**EntityType 枚举（8 种，互斥穷举）**：
...
| relation_type | RelationType | 关系类型枚举 |
| weight | f64 | 关系权重 [0.0, 1.0] |
| evidence | Option\<String\> | 证据文本 |

**RelationType 枚举（10 种，互斥穷举）**：
...
|------|------|------|
| id | String | 社区唯一标识 |
| community_id | String | 社区标识 |
| title | String | 社区标题 |
| summary | String | 社区摘要 |
| entity_ids | Vec\<String\> | 社区内实体 ID 列表 |
| level | usize | 层次聚类层级 |
...
> **注意**：以下为设计文档中的概念示例。实际实现位于 `crates/knowledge-api/src/knowledge_vm.rs`，
> 使用 `knowledge_core::SurrealDbClient` 封装，错误类型使用 `error_core::Result<T>`。
> 代码中的 `KnowledgeError` 已迁移至 error-core 注册表（见下方映射表）。

```rust
// 设计概念示例（非实际代码，实际实现见 knowledge_vm.rs）
...
// 并发安全性论证：
// 1. SurrealDbClient 内部连接池天然支持并发查询（多路复用），无需应用层 Mutex
...
impl KnowledgeVM {
    ...
    pub async fn get_block(&self, id: &str) -> Result<Option<Block>> {
        self.db.select(id).await
    }
    // 精确图遍历：获取 Token 的引用链
    pub async fn trace_references(&self, token_id: &str) -> Result<Vec<Reference>> {
        self.db.query_references(token_id).await
    }
}
```
...
5.3 图形化绑定配置系统（工程级界定）
核心修正：低代码绑定仅限于结构化表单与列表（如文档列表、Token属性面板）。对于图谱的力导向渲染、节点折叠、局部高亮，使用 Dioxus 原生 RSX 组件 + petgraph 图算法实现，图谱的初始数据源通过绑定配置注入。
绑定配置结构 (JSON Schema)：
...
| 代码解析 | tree-sitter | 0.25 | 多语言 AST 解析核心 | GitHub 官方维护，>12k stars，虽<1.0但实质稳定，风险低 ⚠️ |
| 前端框架 | dioxus | ^0.5 | 编译为 WASM 的 UI 框架 | <1.0，活跃开发中。mitigation: UI 层解耦，可迁移至 Leptos/Tauri |
| 图可视化 | Dioxus + petgraph | — | Dioxus 原生组件 + petgraph 图算法 | 自研图谱渲染组件，基于 Dioxus RSX 声明式渲染，无额外框架依赖 ✅ |
...
> **注意**：以下为设计概念示例。实际 `DatabaseClient` trait 定义见 `crates/knowledge-core/src/database.rs`，
> 签名可能因版本迭代而变更。
...
pub trait DatabaseClient: Send + Sync {
    async fn select<T>(&self, id: &str) -> error_core::Result<Option<T>>;
    async fn query_references(&self, token_id: &str) -> error_core::Result<Vec<Reference>>;
}
// 生产实现：SurrealDB 客户端
pub struct SurrealDbClient { /* 内部封装 SurrealDB 连接 */ }
#[async_trait]
...
| 配置键 | 类型 | 默认值 | 说明 | 敏感度 |
|--------|------|-------|------|--------|
| server.port | u16 | 3000 | Axum HTTP 服务端口 | 普通 |
| server.host | string | "127.0.0.1" | 服务监听地址 | 普通 |
| server.tls | bool | false | 是否启用 TLS | 普通 |
| server.cert_path | string? | None | TLS 证书路径（tls=true 时必填） | 🔒机密 |
| server.key_path | string? | None | TLS 私钥路径（tls=true 时必填） | 🔒机密 |
| server.max_request_size | u64 | 10485760 (10MB) | 最大请求体大小 | 普通 |
| database.addr | string | "ws://localhost:8000" | SurrealDB 连接地址 | ⚠️ |
| database.namespace | string | "knowledge" | SurrealDB 命名空间 | 普通 |
| database.database | string | "knowledge" | SurrealDB 数据库名 | 普通 |
| database.username | string | env:KNOWLEDGE_DB_USERNAME | 数据库用户名 | 🔒机密 |
| database.password | string | env:KNOWLEDGE_DB_PASSWORD | 数据库密码 | 🔒机密 |
| security.enc_key_path | string | "./enc.key" | AES-256-GCM 密钥文件路径 | 🔒机密 |
| security.jwt_secret | string | env:KNOWLEDGE_JWT_SECRET | JWT 签名密钥 | 🔒机密 |
| security.jwt_expiry_hours | u8 | 24 | JWT 过期时间（小时） | 普通 |
| security.audit_log_path | string | "./audit.log" | 审计日志文件路径 | ⚠️ |
| parser.max_file_size | u64 | 1073741824 (1GB) | 单文件最大字节数 | 普通 |
| parser.chunk_size | usize | 1000 | 语义分块大小（字符） | 普通 |
| parser.chunk_overlap | usize | 100 | 分块重叠大小（字符） | 普通 |
| parser.enable_code_parsing | bool | true | 是否启用代码解析 | 普通 |
| parser.embedding_dim | usize | 2560 | 向量嵌入维度（GEMMA4-E4B 输出维度） | 普通 |
