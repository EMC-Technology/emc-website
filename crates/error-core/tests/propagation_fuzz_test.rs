...
fn test_recovery_hint() {
    let test_cases: Vec<(&str, RecoveryAction, &str)> = vec![
        ("none_action", RecoveryAction::None, "No action"),
        ("retry_action", RecoveryAction::Retry, "Retry the operation"),
        (
            "fallback_action",
    ...
    for (_name, action, description) in test_cases {
...
fn test_recovery_hint_edge_cases() {
    let hint_empty_params =
        RecoveryHint::new(RecoveryAction::Retry, "Retry operation", HashMap::new());
    assert_eq!(hint_empty_params.action(), &RecoveryAction::Retry);
    assert_eq!(hint_empty_params.description(), "Retry operation");
    assert!(hint_empty_params.params().is_empty());

    let hint_empty_description = RecoveryHint::new(RecoveryAction::Retry, "", HashMap::new());
    assert_eq!(hint_empty_description.action(), &RecoveryAction::Retry);
    ...
    let hint_large_params = RecoveryHint::new(
        RecoveryAction::Retry,
        "Retry operation",
        large_params.clone(),
    );
    assert_eq!(hint_large_params.action(), &RecoveryAction::Retry);
    ...
    let hint_nested_params = RecoveryHint::new(
        RecoveryAction::Retry,
        "Retry operation",
        nested_params.clone(),
    );
    assert_eq!(hint_nested_params.action(), &RecoveryAction::Retry);
...
fn test_retry_config() {
    ...
    for (_name, max_attempts, initial_delay_ms, max_delay_ms, backoff_multiplier, use_jitter) in
        test_cases
    {
...
fn test_retry_config_edge_cases() {
    ...
    assert!(!config_zero.use_jitter());

    // Test with very large values
    let config_large = RetryConfig::new(u32::MAX, u64::MAX, u64::MAX, f64::MAX, true);
...
fn test_propagation_all_combinations() {
    ...
    let params_configs = [HashMap::new(), {
        let mut m = HashMap::new();
        m.insert("delay".to_string(), json!(1000));
        m
    }];
...
fn test_propagation_equality() {
    ...
    let hint1 = RecoveryHint::new(RecoveryAction::Retry, "Retry operation", params1.clone());
    let hint2 = RecoveryHint::new(RecoveryAction::Retry, "Retry operation", params1);
    assert_eq!(hint1, hint2);

    let mut params2 = HashMap::new();
    params2.insert("delay".to_string(), json!(2000));
    let hint3 = RecoveryHint::new(RecoveryAction::Retry, "Retry operation", params2);
    assert_ne!(hint1, hint3);

    let hint4 = RecoveryHint::new(RecoveryAction::Fallback, "Retry operation", HashMap::new());
    assert_ne!(hint1, hint4);

    let hint5 = RecoveryHint::new(
        RecoveryAction::Retry,
