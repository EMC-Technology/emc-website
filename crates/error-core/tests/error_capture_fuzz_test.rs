...
use error_core::error_capture::{
    BusinessErrorCapture, ErrorCapture, ErrorPropagation, FrontendErrorCapture,
    GatewayErrorCapture, InfrastructureErrorCapture,
};
use serde_json::json;
...
fn test_error_capture_all_combinations() {
    ...
    let captures: Vec<(&str, Box<dyn ErrorCapture>)> = vec![
        (
            "frontend",
            Box::new(FrontendErrorCapture::new("user_dashboard")),
        ),
        (
            "gateway",
            Box::new(GatewayErrorCapture::new("auth_service", "login")),
        ),
        (
            "business",
            Box::new(BusinessErrorCapture::new(
                "order_service",
                "process_payment",
            )),
        ),
        (
            "infrastructure",
            Box::new(InfrastructureErrorCapture::new("database", "connection")),
        ),
    ];
    ...
    for (name, capture) in captures {
        for error in errors {
            let captured = capture.capture_error(error);
            assert!(
                !captured.code().is_empty(),
                "{name} capture failed for error"
            );
