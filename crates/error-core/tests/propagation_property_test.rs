...
fn test_context_frame_edge_cases() {
    ...
    assert_eq!(
        frame.data().get("string_key").unwrap(),
        &serde_json::json!("value")
    );
    assert_eq!(
        frame.data().get("number_key").unwrap(),
        &serde_json::json!(42)
    );
    assert_eq!(
        frame.data().get("bool_key").unwrap(),
        &serde_json::json!(true)
    );
    assert_eq!(
        frame.data().get("array_key").unwrap(),
        &serde_json::json!([1, 2, 3])
    );
    assert_eq!(
        frame.data().get("object_key").unwrap(),
        &serde_json::json!({"nested": "value"})
    );
}
...
fn test_recovery_hint_edge_cases() {
    let data = HashMap::new();
    let hint = RecoveryHint::new(RecoveryAction::None, "", data);
    assert_eq!(hint.action(), &RecoveryAction::None);
    ...
    assert_eq!(hint.action(), &RecoveryAction::Escalate);
    assert_eq!(hint.description(), "Complex description");
    assert_eq!(hint.params().len(), 5);
    assert_eq!(
        hint.params().get("string_param").unwrap(),
        &serde_json::json!("value")
    );
    assert_eq!(
        hint.params().get("number_param").unwrap(),
        &serde_json::json!(42)
    );
    assert_eq!(
        hint.params().get("bool_param").unwrap(),
        &serde_json::json!(true)
    );
    assert_eq!(
        hint.params().get("array_param").unwrap(),
        &serde_json::json!([1, 2, 3])
    );
    assert_eq!(
        hint.params().get("object_param").unwrap(),
        &serde_json::json!({"nested": "value"})
    );
}
