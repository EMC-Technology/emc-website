...
fn test_error_code_valid_cases() {
    ...
    for (source, module, sequence, severity, impact_scope, expected_code) in valid_cases {
        ...
        assert!(
            result.is_ok(),
            "Failed to create error code for source={source:?}, module={module}, sequence={sequence}, severity={severity:?}, impact_scope={impact_scope:?}"
        );
        ...
        assert!(
            parse_result.is_ok(),
            "Failed to parse code '{expected_code}'"
        );
...
fn test_error_code_invalid_cases() {
    ...
    for (source, module, sequence, severity, impact_scope, description) in invalid_cases {
        ...
        assert!(
            result.is_err(),
            "Expected error for {description}: source={source:?}, module={module}, sequence={sequence}, severity={severity:?}, impact_scope={impact_scope:?}"
        );
    }
}
...
fn test_error_code_parse_invalid_cases() {
    ...
    let invalid_codes = vec![
        ...
        (
            "ERR-AIM",
            "Missing module, sequence, severity, and impact scope",
        ),
    ...
    for (code, description) in invalid_codes {
        ...
        assert!(
            result.is_err(),
            "Expected error for {description}: '{code}'"
        );
    }
}
...
fn test_error_code_sequence_boundaries() {
    ...
    for sequence in boundary_sequences {
        ...
        assert!(
            parse_result.is_ok(),
            "Failed to parse code for sequence={sequence}"
        );
        let parsed_code = parse_result.unwrap();
...
fn test_error_code_module_boundaries() {
    ...
    for module in valid_modules {
        ...
        assert!(
            parse_result.is_ok(),
            "Failed to parse code for module={module}"
        );
        let parsed_code = parse_result.unwrap();
        ...
        for module in &modules {
            for severity in &severities {
                for impact_scope in &impact_scopes {
                    ...
                    assert!(
                        result.is_ok(),
                        "Failed for source={source:?}, module={module}, severity={severity:?}, impact_scope={impact_scope:?}"
                    );
                    let error_code = result.unwrap();
