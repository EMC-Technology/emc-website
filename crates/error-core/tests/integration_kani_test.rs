...
fn test_complete_error_handling_flow() {
    ...
    let propagated_error =
        ErrorPropagation::append_context(captured_error, "api_gateway", propagation_context);
    ...
    let user_prompt = prompt_manager.get_error_prompt(
        propagated_error.severity(),
        propagated_error.message(),
        None,
    );
...
fn test_logging_and_log_entry() {
    ...
    assert_eq!(
        log_entry.impact_scope(),
        crate::classification::ImpactScope::SESSION
    );
    assert_eq!(
        log_entry.recoverability(),
        crate::classification::Recoverability::AutoRecoverable
    );
    ...
    assert_eq!(log_entry.level(), crate::logging::LogLevel::Error);
}
