...
use error_core::user_prompt::{UserPromptConfig, UserPromptManager, TerminalUIPromptAdapter};
...
fn test_user_prompt_config_templates() {
    ...
    // Test Chinese locale
    assert_eq!(config.get_prompt(Severity::CRITICAL, test_message, Some("zh-CN")), "严重错误: Test message");
    assert_eq!(config.get_prompt(Severity::ERROR, test_message, Some("zh-CN")), "错误: Test message");
    assert_eq!(config.get_prompt(Severity::WARNING, test_message, Some("zh-CN")), "警告: Test message");
    assert_eq!(config.get_prompt(Severity::INFO, test_message, Some("zh-CN")), "信息: Test message");
    
    // Test French locale
    assert_eq!(config.get_prompt(Severity::CRITICAL, test_message, Some("fr-FR")), "Erreur critique: Test message");
    assert_eq!(config.get_prompt(Severity::ERROR, test_message, Some("fr-FR")), "Erreur: Test message");
    assert_eq!(config.get_prompt(Severity::WARNING, test_message, Some("fr-FR")), "Avertissement: Test message");
    assert_eq!(config.get_prompt(Severity::INFO, test_message, Some("fr-FR")), "Information: Test message");
    
    // Test default locale (en-US)
    assert!(config.get_prompt(Severity::CRITICAL, test_message, Some("en-US")).contains("critical error"));
    assert!(config.get_prompt(Severity::ERROR, test_message, Some("en-US")).contains("error occurred"));
    assert!(config.get_prompt(Severity::WARNING, test_message, Some("en-US")).contains("warning occurred"));
    assert!(config.get_prompt(Severity::INFO, test_message, Some("en-US")).contains("Information: Test message"));
}
...
fn test_user_prompt_config_render_template() {
    ...
    config.set_template(Severity::ERROR, "en-US", "Error: {message}. Please see {message} for details.");
    
    let test_message = "Network timeout";
    let prompt = config.get_prompt(Severity::ERROR, test_message, Some("en-US"));
    assert_eq!(prompt, "Error: Network timeout. Please see Network timeout for details.");
}
...
fn test_user_prompt_config_set_default_prompt() {
    ...
    assert_eq!(config.get_prompt(Severity::CRITICAL, test_message, None), "Custom critical error message");
    assert_eq!(config.get_prompt(Severity::ERROR, test_message, None), "Custom error message");
    assert_eq!(config.get_prompt(Severity::WARNING, test_message, None), "Custom warning message");
    assert_eq!(config.get_prompt(Severity::INFO, test_message, None), "Custom info message: Test message");
}
