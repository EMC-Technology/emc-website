...
fn test_user_prompt_config() {
    ...
    for (_name, locale) in test_cases {
        ...
        // Test set_template
        config.set_template(Severity::ERROR, locale, &format!("Error in {}: {{message}}", locale));
        config.set_template(Severity::WARNING, locale, &format!("Warning in {}: {{message}}", locale));
        ...
        let info_prompt = config.get_prompt(Severity::INFO, "Test info", Some("non-existent-locale"));
        assert!(info_prompt.contains("Test info"));
        
        // Test get_prompt with ERROR severity and non-existent locale
        let error_prompt = config.get_prompt(Severity::ERROR, "Test error", Some("non-existent-locale"));
        assert!(!error_prompt.contains("Test error"));
    }
}
...
fn test_user_prompt_config_edge_cases() {
    ...
    let default_prompt = config_no_templates.get_prompt(Severity::ERROR, "Test error", Some("zh-CN"));
...
fn test_user_prompt_manager_edge_cases() {
    ...
    let severities = [Severity::CRITICAL, Severity::ERROR, Severity::WARNING, Severity::INFO];
...
fn test_user_prompt_all_combinations() {
    ...
    for severity in &severities {
        for locale in &locales {
            config.set_template(*severity, locale, &format!("{:?} in {}: {{message}}", severity, locale));
        }
    }
...
fn test_user_prompt_template_rendering() {
    ...
    let templates = [
        ...
        ("Template with multiple variables", "Error: {message} - Please contact support"),
