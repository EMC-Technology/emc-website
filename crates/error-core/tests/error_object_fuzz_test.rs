...
fn test_error_object_optional_fields() {
    ...
    assert_eq!(
        error.details().get("key1"),
        Some(&serde_json::json!("value1"))
    );
    assert_eq!(error.details().get("key2"), Some(&serde_json::json!(42)));
    assert_eq!(
        error.details().get("key3"),
        Some(&serde_json::json!([1, 2, 3]))
    );
    assert_eq!(
        error.details().get("key4"),
...
fn test_error_object_cause_chain() {
    ...
    assert_eq!(result.cause().unwrap().code(), "ERR-NET-API-001_ERR_S");

    let mut error_mut = result;
        ...
        for severity in &severities {
            for impact_scope in &impact_scopes {
                for recoverability in &recoverabilities {
                    ...
                    let _error = ErrorObject::builder()
                        .code(&code)
                        .source(*source)
                        .severity(*severity)
                        .impact_scope(*impact_scope)
