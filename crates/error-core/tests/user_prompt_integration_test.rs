...
fn test_user_prompt_integration() {
    ...
    let error_prompt = prompt_manager.get_error_prompt(Severity::ERROR, "User not found", Some("zh-CN"));
    assert!(!error_prompt.is_empty());

    // 测试不同级别的提示
    let warning_prompt = prompt_manager.get_warning_prompt("Password will expire soon", Some("zh-CN"));
    ...
    let configured_prompt = configured_manager.get_error_prompt(Severity::ERROR, "User not found", Some("zh-CN"));
