...
use error_core::logging::{LogEntry, LogFileManager, LogLevel, LoggingUtils};
...
fn test_log_file_manager() {
    ...
    for log_dir in log_dirs {
        ...
        assert_eq!(
            manager.get_log_path(&Level::ERROR),
            format!("{log_dir}/error.log").replace("//", "/")
        );
        assert_eq!(
            manager.get_log_path(&Level::WARN),
            format!("{log_dir}/warning.log").replace("//", "/")
        );
        assert_eq!(
            manager.get_log_path(&Level::INFO),
            format!("{log_dir}/info.log").replace("//", "/")
        );
        assert_eq!(
            manager.get_log_path(&Level::DEBUG),
            format!("{log_dir}/debug.log").replace("//", "/")
        );
        assert_eq!(
            manager.get_log_path(&Level::TRACE),
            format!("{log_dir}/trace.log").replace("//", "/")
        );

        // Test get_log_path_from_level with all levels
        assert_eq!(
