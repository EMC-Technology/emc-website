...
use error_core::logging::{LoggingUtils, LogFileManager};
use error_core::recovery::{RecoveryStateMachine, CircuitBreaker};
use error_core::propagation::{ContextFrame, RecoveryHint, RetryConfig};
use error_core::error_capture::{BusinessErrorCapture, InfrastructureErrorCapture, GatewayErrorCapture, FrontendErrorCapture};
use error_core::user_prompt::{UserPromptManager, UserPromptConfig};
...
fn test_full_error_lifecycle_e2e() {
    ...
    // 步骤3: 传播错误，附加上下文
    let propagated_error = error_core::error_capture::ErrorPropagation::wrap_error(business_error, db_error);
    ...
    let user_prompt = prompt_manager.get_error_prompt(Severity::ERROR, "Failed to retrieve user data", Some("zh-CN"));
    ...
    // 模拟重试失败
    recovery_state.recover_failed();
    recovery_state.recover_failed();
    recovery_state.recover_failed();
    
    assert_eq!(*recovery_state.state(), error_core::recovery::RecoveryState::Failed);
}
...
fn test_recovery_mechanism_e2e() {
    ...
    // 模拟第二次重试成功
    recovery_state.recover_success();
    assert_eq!(*recovery_state.state(), error_core::recovery::RecoveryState::Recovered);
    ...
    // 断路器应该打开
    assert_eq!(*circuit_breaker.state(), error_core::recovery::CircuitBreakerState::Open);
...
fn test_user_prompt_e2e() {
    ...
    // 测试英文提示
    let en_prompt = prompt_manager.get_error_prompt(Severity::ERROR, "Network error", Some("en-US"));
    assert!(!en_prompt.is_empty());
    
    // 测试中文提示
    let zh_prompt = prompt_manager.get_error_prompt(Severity::ERROR, "Network error", Some("zh-CN"));
...
fn test_error_object_builder_e2e() {
    ...
    let error = ErrorObject::builder()
        ...
        .recovery_hint(RecoveryHint::new("Check username and try again", "Please verify your username and password", hint_params))
        .build();
