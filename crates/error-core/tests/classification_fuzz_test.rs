    ...
    for (input, expected, description) in sources {
        ...
        match expected {
            Ok(expected_source) => {
                ...
                assert_eq!(
                    parsed, expected_source,
                    "Parsed '{input}' as {parsed:?}, expected {expected_source:?} ({description})"
                );
                assert_eq!(
                    parsed.as_str(),
                    input,
                    "as_str() returned wrong value for '{input}' ({description})"
                );
                assert_eq!(
                    format!("{parsed}"),
                    input,
                    "Display returned wrong value for '{input}' ({description})"
                );
            }
            Err(_) => {
                assert!(
                    result.is_err(),
                    "Expected error for '{input}' ({description})"
                );
            }
        }
    }
    ...
    for (input, expected, description) in severities {
        ...
        match expected {
            Ok(expected_severity) => {
                ...
                let parsed = result.unwrap();
                assert_eq!(
                    parsed, expected_severity,
                    "Parsed '{input}' as {parsed:?}, expected {expected_severity:?} ({description})"
                );
                assert_eq!(
                    parsed.as_str(),
                    input,
    ...
    for (input, expected, description) in scopes {
        ...
        match expected {
            Ok(expected_scope) => {
                ...
                let parsed = result.unwrap();
                assert_eq!(
                    parsed, expected_scope,
                    "Parsed '{input}' as {parsed:?}, expected {expected_scope:?} ({description})"
                );
                assert_eq!(
                    parsed.as_str(),
                    input,
...
fn test_recoverability_fuzzing() {
    ...
    let recoverabilities = vec![
        ...
        (
            "MANUAL",
            Ok(Recoverability::ManualIntervention),
            "Valid MANUAL",
        ),
    ...
    for (input, expected, description) in recoverabilities {
        ...
        match expected {
            Ok(expected_recoverability) => {
                ...
                let parsed = result.unwrap();
                assert_eq!(
                    parsed, expected_recoverability,
                    "Parsed '{input}' as {parsed:?}, expected {expected_recoverability:?} ({description})"
                );
                assert_eq!(
                    parsed.as_str(),
                    input,
...
fn test_enum_hash() {
    ...
    assert_eq!(
        scope_map.get(&ImpactScope::SESSION),
        Some(&"Session impact")
    );
    ...
    assert_eq!(
        recoverability_map.get(&Recoverability::AutoRecoverable),
        Some(&"Auto recoverable")
    );
}
...
fn test_from_str_edge_cases() {
    ...
    // Test valid inputs
    assert!(
        ErrorSource::from_str("USR").is_ok(),
        "Failed to parse 'USR'"
    );
    assert!(
        ErrorSource::from_str("AIM").is_ok(),
        "Failed to parse 'AIM'"
    );
    ...
    assert!(
        Recoverability::from_str("AUTO").is_ok(),
        "Failed to parse 'AUTO'"
    );
    assert!(
        Recoverability::from_str("SEMI").is_ok(),
        "Failed to parse 'SEMI'"
    );

    // Test invalid inputs
    assert!(
        ErrorSource::from_str("").is_err(),
        "Expected error for empty string"
    );
    assert!(
        ErrorSource::from_str("invalid").is_err(),
    ...
    assert!(
        Severity::from_str("invalid").is_err(),
        "Expected error for 'invalid'"
    );
    assert!(
        ImpactScope::from_str("").is_err(),
        "Expected error for empty string"
    );
    assert!(
        ImpactScope::from_str("invalid").is_err(),
        "Expected error for 'invalid'"
    );
    assert!(
        Recoverability::from_str("").is_err(),
        "Expected error for empty string"
    );
    assert!(
        Recoverability::from_str("invalid").is_err(),
        "Expected error for 'invalid'"
    );
}
