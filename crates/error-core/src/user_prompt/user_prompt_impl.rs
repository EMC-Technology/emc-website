...
impl UserPromptConfig {
    ...
    pub fn new() -> Self {
        ...
        config.set_default_prompt(
            Severity::WARNING,
            "A warning occurred. Please review your actions.",
        );
    ...
    pub fn set_template(&mut self, severity: Severity, locale: &str, template: &str) {
        self.templates
            .insert((severity, locale.to_string()), template.to_string());
    }
    ...
    pub fn get_prompt(&self, severity: Severity, message: &str, locale: Option<&str>) -> String {
        ...
        self.templates
            ...
            .map_or_else(
                || {
                    self.default_prompts.get(&severity).map_or_else(
                        || message.to_string(),
                        |prompt| Self::render_template(prompt, message),
                    )
                },
...
mod tests {
    ...
    fn test_terminal_ui_prompt_all_severities() {
        ...
        // 测试所有严重程度的提示
        let critical_prompt =
            adapter.get_terminal_prompt(Severity::CRITICAL, "Critical error", None);
