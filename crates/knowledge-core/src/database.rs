...
pub trait DatabaseClient: Send + Sync {
    ...
    /// # Errors
    /// 记录不存在或数据库连接失败时返回错误
    async fn select<T: for<'de> serde::Deserialize<'de> + Send>(
        &self,
        id: RecordId,
    ) -> Result<Option<T>>;
    ...
    ///
    /// # Errors
    /// SQL 语法错误或数据库连接失败时返回错误
    async fn query(&self, sql: &str, bindings: impl serde::Serialize + Send) -> Result<Vec<Value>>;

    /// 在指定表中创建记录
    ///
    /// # Errors
    /// 数据库连接失败或约束违反时返回错误
    async fn create<T: serde::Serialize + Send>(&self, table: &str, data: T) -> Result<Vec<Value>>;

    /// 按 RecordId 更新记录
    ///
    /// # Errors
    /// 记录不存在或数据库连接失败时返回错误
    async fn update<T: serde::Serialize + Send>(
        &self,
        id: RecordId,
        data: T,
    ) -> Result<Option<Value>>;
    ...
    /// # Errors
    /// 数据库连接失败时返回错误
    async fn delete(&self, id: RecordId) -> Result<Option<Value>>;

    /// 批量插入记录
    ///
    /// # Errors
    /// 数据库连接失败或约束违反时返回错误
    async fn insert_batch(&self, table: &str, items: Vec<serde_json::Value>) -> Result<Vec<Value>>;
...
impl DatabaseClient for MockDbClient {
    ...
    async fn delete(&self, _id: RecordId) -> Result<Option<Value>> {
        Ok(Some(Value::None))
    }
    ...
    async fn execute_transaction(
        ...
        for (q, b) in queries.into_iter().zip(bindings.into_iter()) {
